...
class PROIE():
    ...
    def _roi_extract(self):
        ...
        self.roi_img = self.align_img[self.uy:self.ly + 62, self.ux:self.lx + 62]
    ...
    def extract_roi(self, path_in_img, rotate=False):
        ...
        #convert 
        self.in_img_c = cv2.cvtColor(self.in_img_c, cv2.COLOR_BGR2GRAY)

        cv2.imshow("farm", self.in_img_c)
        cv2.waitKey(0)
        #-------------
        ...
        self._threshold()
        cv2.imshow("farm", self.thresh_img)
        cv2.waitKey(0)
        self._contours()
        self._landmarks()
